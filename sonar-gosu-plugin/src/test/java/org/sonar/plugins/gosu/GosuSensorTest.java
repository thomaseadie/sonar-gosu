...
public class GosuSensorTest {
  ...
  private void testMetrics(boolean headerComment, int expectedCommentMetric) throws IOException {
    settings.appendProperty(GosuPlugin.IGNORE_HEADER_COMMENTS, "" + headerComment);
    File sourceDir = new File("src/test/resources/org/sonar/plugins/gosu/gmetrics");
    SensorContextTester context = SensorContextTester.create(new File(""));
  ...
  public void compute_coupling_metrics() throws IOException {
    ...
    DefaultInputDir org = addFileWithParentFolder("src/test/resources/org/sonar/plugins/gosu/gmetricswithcoupling/org", "Greeting.groovy");
    // package 'org.foo' contains class 'Foo', used by class 'Greeting', and using class 'Bar'
    DefaultInputDir org_foo = addFileWithParentFolder("src/test/resources/org/sonar/plugins/gosu/gmetricswithcoupling/org/foo", "Foo.groovy");
    // package 'org.bar' contains class 'Bar', used by classes 'Greeting' and 'Foo', but using no other class
    DefaultInputDir org_bar = addFileWithParentFolder("src/test/resources/org/sonar/plugins/gosu/gmetricswithcoupling/org/bar", "Bar.groovy");
  ...
  private static void assertCouplingMeasureAre(SensorContextTester context, String key, Object afferentTot, Object afferentAvg, Object efferentTot, Object efferentAvg) {
    assertThat(context.measure(key, GosuMetrics.AFFERENT_COUPLING_TOTAL.key()).value()).isEqualTo(afferentTot);
    assertThat(context.measure(key, GosuMetrics.AFFERENT_COUPLING_AVERAGE.key()).value()).isEqualTo(afferentAvg);

    assertThat(context.measure(key, GosuMetrics.EFFERENT_COUPLING_TOTAL.key()).value()).isEqualTo(efferentTot);
    assertThat(context.measure(key, GosuMetrics.EFFERENT_COUPLING_AVERAGE.key()).value()).isEqualTo(efferentAvg);
  }
  ...
  public void test_toString() {
    assertThat(sensor.toString()).isEqualTo("GosuSensor");
  }
